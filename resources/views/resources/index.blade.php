...
<div class="space-y-6 animate-fade-in">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        <a href="{{ route('resources.show', $resource) }}" class="glass rounded-xl overflow-hidden card-hover block group">
            <div class="relative h-44 bg-gradient-to-br from-forest/10 to-forest/5 flex items-center justify-center overflow-hidden border-b border-forest/5 shadow-inner-sm">
                @if($resource->cover_image_url)
                {{-- Ambient blurred background glow from the cover itself --}}
                <img src="{{ $resource->cover_image_url }}" alt="" class="absolute inset-0 w-full h-full object-cover blur-xl opacity-35 scale-110 pointer-events-none transition-opacity duration-300 group-hover:opacity-45">
                {{-- Beautifully proportioned portrait book cover with a crisp 3D shadow --}}
                <img src="{{ $resource->cover_image_url }}" alt="{{ $resource->title }}" class="h-36 w-auto aspect-[2/3] object-contain rounded-md shadow-[0_12px_24px_rgba(26,59,43,0.22)] border border-white/20 z-10 transform group-hover:scale-105 group-hover:-translate-y-1 transition-all duration-300">
                @else
                <div class="w-12 h-12 rounded-full bg-forest/5 flex items-center justify-center group-hover:scale-105 transition-transform duration-300">
                    <svg class="w-6 h-6 text-forest/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>
                </div>
                @endif
            </div>
            <div class="p-4">
